...
const CertificateOverview = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const loaded = (await getAllStudents()) as Student[];
        if (!mounted || !loaded) return;
        setStudents(loaded);
        const unique = Array.from(
          new Set(loaded.map((s: Student) => s.studyGroup).filter(Boolean))
        ) as string[];
  ...
  const handleGenerateForStudent = async () => {
    if (!selectedStudent) {
      setSnackbarMessage(
        t('pages.certificates.error.student') ||
          'Bitte einen Studenten auswählen'
      );
    ...
    try {
      ...
      setSnackbarMessage(
        t('pages.certificates.success.download') || 'Zeugnis heruntergeladen'
      );
      ...
      setSnackbarMessage(
        t('pages.certificates.error.student') ||
          'Fehler beim Generieren des Zeugnisses'
      );
      setSnackbarColor('danger');
      setSnackbarOpen(true);
    } finally {
  ...
  const handleGenerateForStudyGroup = async () => {
    ...
    try {
      ...
      setSnackbarMessage(
        t('pages.certificates.success.downloadGroup') ||
          'ZIP mit Zeugnissen heruntergeladen'
      );
      setSnackbarColor('success');
      setSnackbarOpen(true);
    } catch (err: unknown) {
      ...
      setSnackbarMessage(
        t('pages.certificates.error.group') ||
          'Fehler beim Generieren der Zeugnisse'
      );
      setSnackbarColor('danger');
      setSnackbarOpen(true);
    } finally {
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 2, flexWrap: 'wrap' }}>
        ...
        <SearchSelect
          ...
          getOptionLabel={(student: Student) =>
            `${student.firstName ?? ''} ${student.lastName ?? ''} (${student.matriculationId ?? ''}) — ${student.studyGroup ?? ''}`.trim()
          }
          isOptionEqualToValue={(option: Student, val: Student) =>
            option?.id === val?.id
          }
          placeholder={t('pages.certificates.single.placeholder', {
            defaultValue: 'Student auswählen...',
          })}
          aria-label={t('pages.certificates.single.placeholder', {
            defaultValue: 'Student auswählen...',
          })}
          fullWidth={false}
          size="md"
        ...
        <Button
          ...
          sx={{ height: 40 }}
        >
          {t('pages.certificates.single.generate', {
            defaultValue: 'Zeugnis erzeugen',
          })}
        </Button>
        ...
        <SearchSelect
          ...
          placeholder={t('pages.certificates.group.placeholder', {
            defaultValue: 'Studiengruppe auswählen...',
          })}
          aria-label={t('pages.certificates.group.placeholder', {
            defaultValue: 'Studiengruppe auswählen...',
          })}
          fullWidth={false}
          size="md"
        ...
        <Button
          ...
          sx={{ height: 40 }}
        >
          {t('pages.certificates.group.generate', {
            defaultValue: 'Zeugnisse für Gruppe erzeugen',
          })}
        </Button>
      </Box>
